...
from config.config_all import (
    LOG_OKX, OURS_SHOP, COMPLETE_RATE,)
...
async def get_p2p_okx():
    try:
        ...
        result["creator.completionRate"] = result["creator.completionRate"].astype(float)
        ...
        result = result[["creator.nickName", "creator.type", "creator.completionRate", "unitPrice", "minOrderSize", "maxOrderSize", "side"]]

        # print(result)
        # price_fillter = get_filter_price_okx(result)
        # print(price_fillter)
    except Exception as err:
        Logger.warn("Error From gp_okx.py : ",err)
        result = DataFrame([])
    return result
...
async def final_price_okx():
        ...
        for side in ['buy', 'sell']:
            info = p2p_data_okx[side].iloc[0] if side == 'buy' else p2p_data_okx[side].iloc[-1]
            values = {
                "exchange_id": int(5),
