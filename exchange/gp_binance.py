...
async def fetch_all_p2p_binance(trade_type, asset):
    try:
        ...
        logger.warn("fetch_all_p2p_binance")
...
async def get_p2p_binance(trade_type: str, asset: str):
    try:
        ...
        result["adv.asset"] = result["adv.asset"].astype(str)
    ...
    try:
        ...
        for _, row in DataFrame(
            ...
            values = {
                ...
                "symbol": str(p2p_data_row_1["adv.asset"].iloc[0]) + "_THB",
                "price": float(p2p_data_row_1["adv.price"].iloc[0]),
