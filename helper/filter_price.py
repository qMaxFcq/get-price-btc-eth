...
def get_filter_price_binance(filter_limit: dict, p2p_data: DataFrame = None) -> tuple[DataFrame, DataFrame]:
    for i, row in filter_limit.items():
        filter_row1_min = float(row["row1"]["min"])

        p2p_data_row_1 = p2p_data.loc[(p2p_data["adv.dynamicMaxSingleTransAmount"] - p2p_data["adv.minSingleTransAmount"] > 500000)
...
def get_filter_price_okx(p2p_data: DataFrame = None) -> tuple[DataFrame]:
    # Define a constant for the order size difference threshold
    ORDER_SIZE_THRESHOLD = 100000

    # Filter data for buy and sell sides separately
